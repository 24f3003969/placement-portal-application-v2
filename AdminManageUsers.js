...
const AdminManageUsers = {
    ...
    <div class="container-fluid mt-2 pb-5">
        ...
        <div v-if="currentTab === 'students'">
                                    ...
                                    <tbody>
                                        <tr v-for="student in disabledStudents" :key="student.user_id">
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-sm btn-success" @click="openEnableStudentModal(student)">Enable</button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
            ...
            <div class="modal fade" id="enableStudentModal" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Enable {{ selectedStudent?.name }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <label class="form-label">Reactivation Note / Reason (Optional)</label>
                            <textarea v-model="note" class="form-control"></textarea>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" class="btn btn-success" @click="confirmEnableStudent">Confirm Enable</button>
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <tbody>
                                        <tr v-for="company in approvedCompanies" :key="company.id" :class="{ 'table-danger': company.has_response_delay }" :title="company.has_response_delay ? 'So many delays in responding to applications' : ''">
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-sm btn-danger" @click="openManageCompanyModal(company, 'disabled')">Disable</button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        <tr v-for="company in disabledCompanies" :key="company.id">
                                            ...
                                            <td>
                                                <button class="btn btn-sm btn-info me-1" @click="viewCompanyProfile(company)">Profile</button>
                                                <button class="btn btn-sm btn-success" @click="openManageCompanyModal(company, 'enabled')">Re-Enable</button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
        ...
        <div class="modal fade" id="manageCompanyModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ companyAction === 'disabled' ? 'Disable' : 'Enable' }} {{ selectedCompany?.company_name }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <label class="form-label">Reason / Note (Optional)</label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" :class="companyAction === 'disabled' ? 'btn btn-danger' : 'btn btn-success'" @click="confirmUpdateCompanyStatus">
                            Confirm {{ companyAction === 'disabled' ? 'Disable' : 'Enable' }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    data() {
        return {
            ...
            enableStudentModal: null,
            studentAppsModal: null,
            selectedCompany: null,
            companyAction: '',
            manageCompanyModal: null,
            allCompanies: [],
    ...
    methods: {
        ...
        openEnableStudentModal(student) {
            this.selectedStudent = student;
            this.note = '';
            this.enableStudentModal.show();
        },
        async confirmEnableStudent() {
            const res = await fetch(`/api/admin/student/${this.selectedStudent.user_id}/manage`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authentication-Token': localStorage.getItem('token') },
                body: JSON.stringify({ action: 'enable', note: this.note })
            });
            if (res.ok) {
                alert('Student enabled.');
                this.enableStudentModal.hide();
                this.fetchAllData();
            } else {
        ...
        openManageCompanyModal(company, action) {
            this.selectedCompany = company;
            this.companyAction = action;
            this.note = '';
            this.manageCompanyModal.show();
        },
        async confirmUpdateCompanyStatus() {
            const res = await fetch(`/api/admin/company/${this.selectedCompany.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Authentication-Token': localStorage.getItem('token') },
                body: JSON.stringify({ status: this.companyAction, note: this.note })
            });
            if (res.ok) {
                alert(`Company status updated to ${this.companyAction}.`);
                this.manageCompanyModal.hide();
                this.fetchAllData();
            } else {
    ...
    mounted() {
        ...
        this.enableStudentModal = new bootstrap.Modal(document.getElementById('enableStudentModal'));
        ...
        this.manageCompanyModal = new bootstrap.Modal(document.getElementById('manageCompanyModal'));
