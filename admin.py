...
class AdminStudentManagementAPI(Resource):
    ...
    def post(self, user_id):
        ...
        if action == 'disable':
            ...
            create_notification(
                user.id,
                f"Your account has been suspended by the admin. Reason: {note}",
                "warning"
            )
            # Send disable email via Celery
            from application.tasks import send_student_disabled_email_task
            send_student_disabled_email_task.delay(user.id, note)
            ...
            send_student_reactivation_email_task.delay(user.id, note)
