        ...
        <div class="card h-100 shadow-sm border-0 rounded-4 p-1" style="background-color: #dbc9d5;">
            <div class="card-body d-flex flex-column">
                ...
                <div class="d-flex flex-wrap text-muted mb-3 gap-1" style="font-size: 0.8rem;">
                    ...
                    <span v-show="(currentUserRole==='comp' || currentUserRole=='admin') && (drive.Status==='Active' || drive.Status==='Application Closed')" class="badge bg-primary me-2">Total: {{driveStats.total_applicants}}</span>
                    <span v-show="(currentUserRole==='comp' || currentUserRole=='admin') && (drive.Status==='Active' || drive.Status==='Application Closed')" class="badge bg-warning text-dark me-2">Shortlisted: {{driveStats.shortlisted}}</span>
                    <span v-show="(currentUserRole==='comp' || currentUserRole=='admin') && (drive.Status==='Active' || drive.Status==='Application Closed')" class="badge bg-success">Hired: {{driveStats.hired}}</span>
                </div>
